...
pub fn setup_ui_profiler(ui: &mut egui::Ui, profiling_data: &[wgpu_profiler::GpuTimerQueryResult], levels_default_open: i32) {
    for scope in profiling_data.iter() {
        ...
        if scope.nested_queries.is_empty() {
            ...
            egui::CollapsingHeader::new(format!("{}  -  {}", scope.label, time))
                ...
                .show(ui, |ui| setup_ui_profiler(ui, &scope.nested_queries, levels_default_open - 1));
        }
        ui.end_row();
    }
}
